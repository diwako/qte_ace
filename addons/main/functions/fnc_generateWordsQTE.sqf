...
private _qteType = missionNamespace getVariable format ["qte_ace_%1_qtrType", _component];

if (_qteType == 2 || {_qteType == 0 && (floor random 2) isEqualTo 0}) then {
    private _words = missionNamespace getVariable format ["qte_ace_%1_words", _component];
